...
function changePosition(pl_id, t_id, c_id, role, position) {
	...
	$("#page.ui-page-active #" + pl_id + '_' + position).show();
	//$("#" + pl_id + '_' + oldPosition).children().html(positionNames[oldPosition]);
	$("#page.ui-page-active #" + pl_id + '_' + oldPosition).hide();		

	//update arrays
	_line_up[pl_id].position = position;

	$("#page.ui-page-active #edit-line-up").val("" + JSON.stringify(_line_up));

	var checks = checkLineUp(_line_up);

	if (checks[0] && checks[1] && checks[2] && checks[3]) {
		$("#page.ui-page-active #line_up_submit").button('enable');
		$(window).unbind();
	}
	else
		$("#page.ui-page-active #line_up_submit").button('disable');
